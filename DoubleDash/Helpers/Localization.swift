...
struct LocalizedStrings {
    ...
    /// Localized string for alert title when username already exists in registration view.
    ...
    /// Localized string explaining the effect of swiping on tiles
    ...
    /// Localized string explaining the merging of tiles during movement.
    ...
    /// Localized string for the game progression title.
    static var gameProgression: String {
        return GlobalSettings.shared.isEnglish ? "4. Game Progression:" : "4. Tiến trình trò chơi:"
    }
    /// Localized string explaining the appearance of a new tile after every swipe.
    ...
    /// Localized string instructing the user to continue merging tiles to increase their score.
    ...
    /// Localized string describing the increasing difficulty as the game board fills up.
    ...
    /// Localized string explaining the conditions under which the game concludes.
    ...
    /// Localized string for the scoring title.
    static var scoring: String {
        return GlobalSettings.shared.isEnglish ? "6. Scoring:" : "6. Tính điểm:"
    }
    /// Localized string explaining how the score is calculated.
    ...
    /// Localized string for the second content of the leaderboard.
    ...
    /// Localized string for the third content of the leaderboard.
    ...
    /// Localized string for the achievements title.
    static var achievements: String {
        return GlobalSettings.shared.isEnglish ? "8. Achievements:" : "8. Thành tích:"
    }
    /// Localized string for the first content of the achievements.
    static var achievementsContent1: String {
        return GlobalSettings.shared.isEnglish ? "After every swipe, a new tile will appear on the board in a random empty spot." : "Sau mỗi lần vuốt, một ô mới sẽ xuất hiện trên bảng ở một vị trí trống ngẫu nhiên."
    }
    /// Localized string for the secound content of the achievements.
    static var achievementsContent2: String {
        return GlobalSettings.shared.isEnglish ? "Continue swiping and merging tiles to increase your score." : "Tiếp tục vuốt và hợp nhất các ô để tăng điểm của bạn."
    }
    /// Localized string for the last content of the achievements.
...
// This section provides localized strings for various UI elements and prompts in the game.
// It includes alerts for registration views, instructions for how to play the game, scoring details,
// leaderboard information, and achievements. The localization supports both English and Vietnamese languages.
// The actual language displayed is determined by the `GlobalSettings.shared.isEnglish` flag.
